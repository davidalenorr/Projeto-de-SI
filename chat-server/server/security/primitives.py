...
usuários (Projeto 2, seções 5 e 6).
...
- Derivação de chaves: HKDF-SHA256, saída de 64 bytes -> chave AES (32 bytes)
  + chave HMAC (32 bytes), exatamente como a Figura 3 do Projeto 2 descreve.
- Cifra simétrica: AES-256-CBC com padding PKCS7 e IV aleatório de 16 bytes
  por mensagem.
- Integridade/autenticidade: HMAC-SHA256, calculado sobre (IV || texto
  cifrado) — Encrypt-then-MAC, na ordem exigida pela seção 6.3.
- Identidade de usuário (registro, login por desafio, autenticação mútua):
  Ed25519, uma das duas famílias permitidas pela especificação (RSA ou ECC),
  com assinatura Ed25519 nativa.
"""
import base64
...
class IntegrityError(Exception):
    """O HMAC recebido não confere com o recalculado: mensagem alterada, chave
    errada ou tentativa de adulteração. A mensagem correspondente deve ser
    descartada sem ser decifrada (seção 6.3, "Cuidado")."""
...
def open_sealed(key_aes: bytes, key_hmac: bytes, envelope: dict) -> bytes:
    ...
    # A ordem importa: o MAC é verificado antes de qualquer decifragem.
